...
//--------------------------------------------------------------------------------
    ...
    // 定义SQL注入类型的枚举类型
    enum SqlInjectionType {
        UNION_SELECT("Union Select 注入"),
        COMMENT("注释语法注入"),
        BOOLEAN("真值注入"),
        ERROR("错误注入"),
        TIME_BASED("基于时间的盲注入"),
        DANGER("危险的SQL注入"),
        FUNC_BASED("基于特定函数的注入");

        // 枚举类型成员变量
        private final String description;

        // 枚举类型构造函数
        SqlInjectionType(String description) {
    ...
    public static Map<String, String> detectSQLInjectionType(String sqlInjectionString) {
        //配置正则检测
        String[][] regexArray = {
                {"(?i).*\\bCREATE\\b.*", SqlInjectionType.DANGER.name()},
                {"(?i).*\\bDROP\\b.*", SqlInjectionType.DANGER.name()},
                {"(?i).*\\bALTER\\b.*", SqlInjectionType.DANGER.name()},
                {"(?i).*\\bUPDATED\\b.*", SqlInjectionType.DANGER.name()},
                {"(?i).*\\bDELETE\\b.*", SqlInjectionType.DANGER.name()},
                {"(?i).*\\bGRANT\\b.*", SqlInjectionType.DANGER.name()},
                {"(?i).*\\bREVOKE\\b.*", SqlInjectionType.DANGER.name()},
                {"(?i).*\\bSLEEP\\b.*", SqlInjectionType.TIME_BASED.name()},
                {"(?i).*\\bBENCHMARK\\b.*", SqlInjectionType.TIME_BASED.name()},
                {"(?i).*\\bPG_SLEEP\\b.*", SqlInjectionType.TIME_BASED.name()},
                {"(?i).*\\bWAITFOR\\b.*", SqlInjectionType.TIME_BASED.name()},
                {"(?i).*\\bDELAY\\b.*", SqlInjectionType.TIME_BASED.name()},
                {"(?i).*\\bUNION\\b.*\\bSELECT\\b.*", SqlInjectionType.UNION_SELECT.name()},
                {"(?i).*\\bUNION\\b.*=.*", SqlInjectionType.UNION_SELECT.name()},
                {"(?i).*\\bOR\\b.*=.*", SqlInjectionType.BOOLEAN.name()},
                {"(AND|and)\\s+(\\S+)\\s*=\\s*\\2", SqlInjectionType.BOOLEAN.name()},
                {"(?i).*\\bDATABASE\\b.*", SqlInjectionType.FUNC_BASED.name()},
                {"(?i).*\\bCOUNT\\b.*", SqlInjectionType.FUNC_BASED.name()},
                {"(?i).*\\bSUBSTRING\\b.*", SqlInjectionType.FUNC_BASED.name()},
                {"(?i).*\\bCAST\\b.*", SqlInjectionType.FUNC_BASED.name()},
                {"(?i).*\\bCONVERT\\b.*", SqlInjectionType.FUNC_BASED.name()},
                {"(?i).*\\bEXEC\\b.*", SqlInjectionType.FUNC_BASED.name()},
                {"(?i).*\\bSP_EXECUTESQL\\b.*", SqlInjectionType.FUNC_BASED.name()},
                {"(?i).*\\bLOAD_FILE\\b.*", SqlInjectionType.FUNC_BASED.name()},
                {"(?i).*\\bUPDATEXML\\b.*", SqlInjectionType.FUNC_BASED.name()},
                {"(?i).*\\bINSERTXML\\b.*", SqlInjectionType.FUNC_BASED.name()},
                {"(?s).*--.*", SqlInjectionType.COMMENT.name()},
                {"(?i).*\\bUPDATEXML\\b.*", SqlInjectionType.ERROR.name()},
                {"(?i).*\\bGROUP\\b.*", SqlInjectionType.ERROR.name()},
                {"(?i).*\\bSELECT\\b.*\\bFROM\\b.*\\bWHERE\\b.*", SqlInjectionType.ERROR.name()},
        };

        Map<Pattern, String> patternToTypeMap = new HashMap<>();

        for (String[] regexInfo : regexArray) {
            String regex = regexInfo[0];
            String type = regexInfo[1];
        ...
        for (Map.Entry<Pattern, String> entry : patternToTypeMap.entrySet()) {
            Pattern pattern = entry.getKey();
            String typeName = entry.getValue();
            Matcher matcher = pattern.matcher(sqlInjectionString);
            if (matcher.matches()) {
                type = type + typeName + ";";
                description = "The SQL Injection uses " + typeName + " to exploit the query.";
                typeNum++;
            }
        }
        if(typeNum > 1){
        ...
        result.put("isSQLi","1");
        result.put("query",sqlInjectionString);
    ...
    public static Map<String,String> isSQLiAnalyzer(final String input){
        Map<String,String> res = new HashMap<>();
        res.put("query",input);
        res.put("isSQLi","0");
        try{
            if (isSQLiImpl1(input)){
                return detectSQLInjectionType(input);
            }
            return res;
        }catch (Exception e) {
            e.printStackTrace();
